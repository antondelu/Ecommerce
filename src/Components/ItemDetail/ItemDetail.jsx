...
export const ItemDetail = (props) => {
  ...
  const agregarCarrito = (event) => {
    ...
    const buscador = estadoGlobal.cart.find(
      (producto) => producto.itemDetalle.id == itemDetalle.id
    );
    if (buscador) {
      if (itemDetalle === buscador.itemDetalle) {
        estadoGlobal.setTotalCarrito(
          estadoGlobal.totalCarrito + cantidadContext * itemDetalle.price
        );
            ...
            <figcaption className="caption">
              ...
              {!!compra ? (
                <div>
                  ...
                  <But funcion={agregarCarrito} texto={"Agregar"} />
                </div>
              ) : (
                <Link to="/cart">
                  {" "}
                  <But funcion={Comprar} texto={"Comprar"} />{" "}
                </Link>
              )}
            </figcaption>
          </figure>
